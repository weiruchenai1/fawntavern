...
internal data class ChatPromptContextSnapshot(
    val loaded: PromptContextLoader.Loaded,
    val image: Bitmap?,
)

internal interface ChatPromptContextDataSource {
...
/** Android/file-backed prompt resources consumed by the chat feature. */
internal class AndroidChatPromptContextDataSource(context: Context) : ChatPromptContextDataSource {
    ...
    override suspend fun load(charFile: String): ChatPromptContextSnapshot =
        ChatPromptContextSnapshot(
            loaded = PromptContextLoader.load(appContext, charFile),
    ...
    override suspend fun ensureDefaultCharacter(
        ...
        return CharacterRepository.ensureDefaultCard(
            appContext,
