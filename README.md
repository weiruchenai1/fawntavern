...
- 支持第三方插件，并在隔离的 QuickJS 进程中运行插件脚本
- 聊天记录、角色数据和配置保存在本地
- 支持本地备份、恢复与应用内更新检查
...
- 首次启动时必须先阅读隐私政策和用户协议；同意前不会初始化统计与远程诊断服务。

## 信息收集披露

FawnTavern 没有自建服务端，聊天记录、角色卡、预设、世界书和附件默认只保存在设备本地。除下列情况外，应用不会收集或上传任何数据：

- **模型、TTS 与图片服务**：你主动发起请求时，完成请求所需的文本、上下文、附件、参数和凭据会发送到你自行配置的服务商。
- **联网搜索**：开启联网搜索后，搜索关键词会发送到你配置的搜索服务商。
- **远程诊断（同意隐私政策后默认开启，可在设置中关闭）**：应用按所在地区通过腾讯 Bugly（中国大陆）或 Google Firebase（其他地区）处理崩溃报告和启动事件，可能包含应用实例标识、设备标识、设备/系统/网络信息、IP 地址、应用版本与构建类型、崩溃堆栈、线程和运行状态及诊断日志。应用还会访问 Cloudflare 确定地区以选择诊断服务，结果缓存 24 小时。应用不会主动把聊天正文、附件内容或 API 密钥作为诊断字段上传。
- **插件**：插件脚本默认在设备的隔离进程中运行；插件自行发起网络请求时，数据会发送到插件指定的目标服务。
- **版本更新检查**：检查更新时会向 GitHub Releases API 发送标准 HTTPS 请求，GitHub 会接收 IP 地址、请求时间、User-Agent 和网络元数据；当前版本和设备 CPU 架构只在本地用于选择安装包。
